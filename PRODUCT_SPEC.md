...
The route prompt is prefilled from the selected item path relative to the selected site's verified root path. A leading `Home` segment is omitted and remaining item-name segments are normalized into lowercase URL slugs. If the item is outside the known site root, the item name supplies a conservative one-segment suggestion. The value remains editable and can be cleared.
