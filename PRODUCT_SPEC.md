...
- **Power publish…** reuses the grouped Traced Publish configuration, optionally adds structural descendants to an explicit **Observed Reference Graph**, recursively discovers supported references, and lets the user review the exact plan. It publishes dependency layers in checkpointed multi-item operations and the selected root in a final batch before running the single configured route/application trace.
...
The Observed Reference Graph is explicitly limited to supported, observable item references rather than claiming knowledge of code-driven or search-driven dependencies. Discovery follows GUIDs from Droplink, Droptree, Multilist, Treelist, General Link, Image, and layout fields, bounded to 500 items and eight reference levels. Power Publish refuses to queue an incomplete graph when either budget is reached or a referenced item cannot be loaded.
...
## v0.9.8 Power Publish checkpoints
...
The selected graph is collapsed into strongly connected components and dependency layers. Independent components in one layer share Sitecore publish operations containing at most 20 root item IDs; a cyclic component remains indivisible even when larger. The selected root is removed from dependency batching and published in one final batch. Every batch stores its Sitecore operation ID and a Raw Experience Edge checkpoint. Item identity, user-selected fields owned by the batch, and reference fields that produced outgoing graph edges must match before the next dependent layer starts. A timeout stops the Power Publish operation and pauses the Operations FIFO. Restart recovery polls saved operation IDs and resumes at the first checkpoint not already matched.

After the root checkpoint, Power Publish runs the same single aggregate Raw Edge, rendered-route, application-response, and Browser DOM stages as Traced Publish. It does not repeat route or application verification for datasource dependencies.
