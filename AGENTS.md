...
The repository has a TypeScript unit-test suite under `test/unit/`. It uses Node's built-in test runner, Node's strict assertion API, and `tsx`, and runs through `npm test`. Current coverage includes XM Cloud server URL normalization, Sitecore reference discovery, collapsed Power Publish scope graph planning, and Sitecore HTTP transport retry, cooldown, cancellation, and log-redaction behavior. The extension-host suite under `test/integration/` uses `@vscode/test-electron` and covers activation, contributed-command registration, configuration defaults, `TransferQueueStore` persistence and recovery, and `TransferProcessor` FIFO, pause/resume, publishing, field-transfer, subtree-checkpoint, journal, and deployment-monitoring behavior through `npm run test:integration`.
