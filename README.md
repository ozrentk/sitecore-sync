...
- **Power publish…** reuses the grouped Traced Publish configuration, optionally seeds an **Observed Reference Graph** with structural descendants, recursively discovers supported item references, and lets you review its exact item plan. It publishes dependency layers in checkpointed multi-item batches and the selected root in a final batch before running the single configured route/application trace.
...
Power Publish uses the same field picker and keeps assertions limited to that structural scope. It has no **Include related items** switch: supported references are always discovered into the explicit graph, where items can be reviewed before queueing. Discovery is blocked rather than silently truncated when its 500-item or eight-level safety budget is reached or an observed target cannot be loaded. Traced Publish records supported references outside its structural scope as advisory Authoring evidence only.
...
Traced- and Power-publish preparation use the same editor-area configuration page rather than a chain of prompts. Publishing mode and applicable scope, verified Sitecore site, editable route, exact application URL, field assertions, and optional per-field CSS selectors are reviewed together before queueing. Fields are added through the searchable picker; only selected assertions remain in the compact list, where they can receive a CSS selector or be removed. Structural descendant fields are discovered only after **Include structural descendants** is enabled. Power Publish then builds and reviews its explicit graph; Traced Publish queues directly. First-time Experience Edge endpoint and token setup remains separate so the token can be validated and stored in VS Code Secret Storage without entering the webview.
...
Power dependencies are grouped by dependency layer and sent to Sitecore in batches of up to 20 independent root item IDs; cyclic components remain together and the selected root stays in a final batch. After each Sitecore operation finishes, raw Experience Edge must expose the batch items plus selected and graph-forming reference fields before the next dependent layer starts. A failed checkpoint stops the plan. Batch operation IDs and checkpoint evidence are persisted so restart recovery does not repeat a matched boundary.
