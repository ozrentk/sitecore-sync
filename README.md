...
Right-click an item and choose **Add Subtree Transfer Left → Right…** or **Add Subtree Transfer Right → Left…**. After explicit confirmation, the extension adds the subtree to the workspace's FIFO queue. Field Diff arrows add field-value transfers to the same queue. Duplicate pending requests are not added twice.
...
Subtree rows show six phases: queued, freshness checking, content export, chunk copying, Sitecore import, and verification. Chunk copying uses the actual Content Transfer count, for example `copying chunks (4/6, chunk 7/23)`, while Item Transfer jobs use `Sitecore (5/6, import 2/3)`. Field-value rows show four phases from queued through verification. Detailed subtree progress is persisted with the queue record and remains visible on failure.
