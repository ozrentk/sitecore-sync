...
Traced-publish preparation uses one editor-area configuration page rather than a chain of prompts. Publishing mode and scope, verified Sitecore site, editable route, exact application URL, field assertions, and optional per-field CSS selectors are reviewed together before **Publish** is selected. The field table is searchable and limits rendered matches for responsiveness, while structural descendant fields are discovered only after **Include structural descendants** is enabled. First-time Experience Edge endpoint and token setup remains separate so the token can be validated and stored in VS Code Secret Storage without entering the webview.
