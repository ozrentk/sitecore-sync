...
Right-click a comparison item and choose **Add to Favorites** to save its Sitecore path beneath one or both participating connections. Favorites are marked with `☆` in the Connections view and can be removed from their context action. Clicking a favorite opens the comparison and selects that item. Already-loaded rows are revealed without an API request; otherwise the extension resolves the path with Authoring GraphQL and loads the ancestor chain needed to display it. If the saved path is unavailable, the error prompt offers to remove the stale favorite.
